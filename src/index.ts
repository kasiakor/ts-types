...
function calcTax3(amount: number, format: boolean): string | number | null {
    if (amount === 0) {
        return null;
    }
    const calcAmount = amount * 1.2;
...
let taxString = calcTax3(200, true) as string;
...
function calcTax4(amount: number, format:boolean): string | number | null {
    if (amount === 0) {
        return null;
    }
    const calcAmount = amount * 1.2;
...
//Value is null
let taxValue2: string | number | null = calcTax4(0, false);
...
function calcTax5(amount: number, format:boolean): string | number | null {
    if (amount === 0) {
        return null;
    }
    const calcAmount = amount * 1.2;
...
//Value is null
let taxValue5: string | number | null = calcTax5(0, false);
if(taxValue5 !== null) {
    let nonNullTaxValue: string | number  = taxValue5;
